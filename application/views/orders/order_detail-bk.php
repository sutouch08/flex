...
<?php 	$netAmount = ( $total_amount - $order->bDiscAmount - $order->deposit ) + $order->shipping_fee + $order->service_fee;	?>
					<?php if(getConfig('ALLOW_BILL_DISCOUNT')) : ?>
						<tr id="billDisc">
							<td colspan="6" class="middle text-right">ส่วนลดท้ายบิล</td>
							<td class="middle">
								<span class="input-icon input-icon-right">
									<input type="number" id="billDiscPercent" class="form-control input-sm" value="<?php echo $order->bDiscText; ?>" disabled/>
									<i class="ace-icon fa fa-percent"></i>
								</span>
							</td>
					<?php endif; ?>
							<td class="middle">
								<input type="text" class="form-control input-sm text-right" id="billDiscAmount" name="billDiscAmount" value="<?php echo $order->bDiscAmount; ?>" disabled/>
								<!-- total amount after row discount but before bill disc -->
								<input type="hidden" id="totalAfDisc" value="<?php echo $netAmount; ?>" />
							</td>
							<td class="middle padding-5 text-right">
								<button type="button" class="btn btn-xs btn-warning" id="btn-edit-bdisc" onclick="active_bdisc()"><i class="fa fa-pencil"></i></button>
								<button type="button" class="btn btn-xs btn-success hide" id="btn-update-bdisc" onclick="update_bdisc()"><i class="fa fa-save"></i></button>
							</td>
						</tr>
						<tr class="font-size-12">
