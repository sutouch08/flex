          ...
          <div class='row'>
            ...
            <?php 	foreach($addr as $rs) : ?>
                ...
                <?php if( $rs->id == $order->address_id ) : ?>
                        <button type="button" class="btn btn-mini btn-success btn-address" style="min-width:30px; margin-bottom:2px;" id="btn-<?php echo $rs->id; ?>" onClick="setDefault(<?php echo $rs->id; ?>)">
                          <i class="fa fa-check"></i>
                        </button>
                <?php else : ?>
                        <button type="button" class="btn btn-mini btn-address" style="min-width:30px; margin-bottom:2px;" id="btn-<?php echo $rs->id; ?>" onClick="setDefault(<?php echo $rs->id; ?>)">
                          <i class="fa fa-check"></i>
                        </button>
                <?php endif; ?>
  											<button type="button" class="btn btn-mini btn-primary" style="min-width:30px; margin-bottom:2px;" onclick="printOnlineAddress(<?php echo $rs->id; ?>, '<?php echo $order->code; ?>')"><i class="fa fa-print"></i></button>
                        <button type="button" class="btn btn-mini btn-warning" style="min-width:30px; margin-bottom:2px;" onClick="editAddress(<?php echo $rs->id; ?>)"><i class="fa fa-pencil"></i></button>
                        <button type="button" class="btn btn-mini btn-danger" style="min-width:30px; margin-bottom:2px;" onClick="removeAddress(<?php echo $rs->id; ?>)"><i class="fa fa-trash"></i></button>
                      </td>
                    </tr>
            <?php 	endforeach; ?>
            <?php else : ?>
